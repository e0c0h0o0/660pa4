...
double IntHistogram::estimateSelectivity(Predicate::Op op, int v) const {
  ...
  switch (op) {
    ...
    case Predicate::Op::GREATER_THAN:
      ...
      candidates = 0;
      if (avg_ > 1) {
        candidates = (double )(bucket->hi_ - v)/bucket->interval() *bucket->in_bucket_;
      }
      //candidates = 0.0;
    ...
    case Predicate::Op::NOT_EQUALS:
      ...
      return ((double )n_tuples_ - (double )bucket->in_bucket_/(double )avg_)/(double )n_tuples_;
    default:
      break;
  }
